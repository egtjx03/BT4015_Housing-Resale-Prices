...
planning <- st_read("/Users/sugarcane/Downloads/bt4015/Datasets/MasterPlan2019PlanningAreaBoundaryNoSea.geojson")

parkConnector <- st_zm(parkConnector)
planning <- st_zm(planning)

parkConnector_3414 <- st_transform(parkConnector, 3414)
planning_3414 <- st_transform(planning, 3414)

coords <- st_coordinates(parkConnector_3414)
kde <- kde2d(
  coords[,1],
  coords[,2],
  n = 300,      
