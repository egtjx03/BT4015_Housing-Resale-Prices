...
schools_clean <- na.omit(schools[, c("longitude", "latitude")])
schools <- st_as_sf(schools_clean, coords = c("longitude", "latitude"), crs = 4326)
planning <- st_zm(planning)

schools_3414 <- st_transform(schools, 3414)
planning_3414 <- st_transform(planning, 3414)

coords <- st_coordinates(schools_3414)
kde <- kde2d(
  coords[,1],
  coords[,2],
  n = 300,     
